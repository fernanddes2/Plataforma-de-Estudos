...
const AIChat: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] max-w-5xl mx-auto p-4">
        {/* Header */}
        <div className="bg-white border border-gray-200 rounded-2xl p-6 mb-4 flex items-center justify-between shadow-sm">
            <div className="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">Especialista em Engenharia Elétrica</p>
                </div>
            </div>
            <button 
                onClick={() => {
                ...
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`flex max-w-[80%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'} items-start gap-3`}>
                        ...
                        <div className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${
                            msg.role === 'user' 
                                ? 'bg-secondary-900 text-white rounded-tr-none' 
        ...
        <div className="bg-white p-2 rounded-2xl border border-gray-200 shadow-lg flex items-end gap-2">
            <textarea
                ...
                className="flex-1 max-h-32 min-h-[3rem] p-3 bg-transparent border-none focus:ring-0 resize-none text-sm text-gray-800 placeholder-gray-400"
                rows={1}
            />
            <button
                onClick={handleSend}
